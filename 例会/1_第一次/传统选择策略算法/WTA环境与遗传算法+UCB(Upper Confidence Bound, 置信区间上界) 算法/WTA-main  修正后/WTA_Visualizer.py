...
class WTA_Monitor:
    def __init__(self):
        self.op_history = {i: [] for i in range(1, 9)}  # 8个算子
        self.div_history = []
    ...
    def update(self, iteration, counts, diversity, rewards):
        ...
        total = sum(counts)
        for i in range(8): self.op_history[i + 1].append(counts[i] / total)
        self.div_history.append(diversity)
        for i in range(8): self.reward_dist[i].extend(rewards[i])
    ...
    def show_all_reports(self, trad_history, ucb_history, trad_div):
        ...
        axs[1, 1].set_title("各算子单次进化收益分布 (对数+生存奖励)")
