...
class Pool:
    ...
    def Process(self, monitor=None, current_iter=0):
        ...
        if div < 0.1:
            self.mutationRate = self.initial_mutation_rate * 2.0
            self.low_diversity_count += 1
        else:
            self.mutationRate = self.initial_mutation_rate
            self.low_diversity_count = 0
        ...
        if self.no_improvement_count >= 15 or self.low_diversity_count >= 10:
            self.Earthquake()
            self.no_improvement_count = 0
            self.low_diversity_count = 0
    ...
    def Earthquake(self):
        keep_num = max(1, int(self.poolSize * 0.1))
...
class Chromosome:
    ...
    def GreedyCorrection(self):
        ...
        for w_idx, t_idx in enumerate(c.chromosome):
            survive_p[t_idx] *= (1 - self.thePool.P_matrix[w_idx][t_idx])
        worst_targets = np.argsort(self.thePool.V_targets * survive_p)[-3:]
        c.chromosome[random.randint(0, self.numberofWeapon - 1)] = random.choice(worst_targets)
        return c

    def LocalSearch(self):
        c = Chromosome(self.thePool, self.mutationRate, self.numberofTarget, self.numberofWeapon)
        c.chromosome = list(self.chromosome)
        for idx in random.sample(range(self.numberofWeapon), min(5, self.numberofWeapon)):
    ...
    def PointMutation(self):
        c = Chromosome(self.thePool, self.mutationRate, self.numberofTarget, self.numberofWeapon);
        c.chromosome = list(self.chromosome)
        if random.random() < self.mutationRate: c.chromosome[
            random.randint(0, self.numberofWeapon - 1)] = random.randint(0, self.numberofTarget - 1)
        return c
